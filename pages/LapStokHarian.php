...
<?php for ($i = 1; $i <= $d; $i++){ 
	...
	$sqlRMasuk = mysqli_query($con," SELECT tgl_tutup,sum(qty) as rol,sum(berat) as kg FROM tblmasukkain 
	...
	$sqlMMasuk = mysqli_query($con," SELECT tgl_tutup,sum(qty) as rol,sum(berat) as kg FROM tblmasukkain 
	...
	$sqlKeluar = mysqli_query($con," SELECT tgl_tutup,sum(qty) as rol,sum(berat) as kg FROM tblkeluarkain 
	WHERE tgl_tutup='$Thn2-$Bln2-$i' AND NOT ISNULL(demand) GROUP BY tgl_tutup");		  
    $rKeluar = mysqli_fetch_array($sqlKeluar);
	$sqlPotong = mysqli_query($con," SELECT tgl_tutup,sum(qty) as rol,sum(berat) as kg FROM tblkeluarkain 
...
FROM tblopname WHERE tgl_tutup='$Thn2-$Bln2-$i' GROUP BY tgl_tutup");		  
