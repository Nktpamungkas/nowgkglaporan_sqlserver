...
$cektgl=sqlsrv_query($con,"SELECT
							...
							FROM
								...
								LEFT JOIN dbnow_gkg.tblmasukkain t ON t.tgl_tutup = target_date.tgl_tutup
							GROUP BY
...
if($dcek['ck']>0){	
	...
	$sqlDB21M = " SELECT
						...
						ITXVIEWLAPMASUKGREIGE.SUBCODE01,
					...
					FROM
						...
						LEFT OUTER JOIN DB2ADMIN.ITXVIEWHEADERKNTORDER ITXVIEWHEADERKNTORDER ON ITXVIEWHEADERKNTORDER.PRODUCTIONORDERCODE = ITXVIEWLAPMASUKGREIGE.EXTERNALREFERENCE
						AND ITXVIEWHEADERKNTORDER.CODE = ITXVIEWLAPMASUKGREIGE.LOTCODE
					WHERE
						STOCKTRANSACTION.TRANSACTIONDATE = '$Awal'
...
$sqlDB22M = "SELECT
				...
				WHERE
					SALESORDER.CODE = '".trim($rowdb21M[' PROJECTCODE '])."'";
				...
				FROM
					(
						...
						WHERE
							...
							AND p.SUBCODE01 = '".trim($rowdb21M[' SUBCODE01 '])."'
							AND p.SUBCODE02 = '".trim($rowdb21M[' SUBCODE02 '])."'
							AND p.SUBCODE03 = '".trim($rowdb21M[' SUBCODE03 '])."'
							AND p.SUBCODE04 = '".trim($rowdb21M[' SUBCODE04 '])."'
							AND (
								p.PROJECTCODE = '".trim($rowdb21M[' PROJECTCODE '])."'
								OR p.ORIGDLVSALORDLINESALORDERCODE = '".trim($rowdb21M[' PROJECTCODE '])."'
							)
					) a
...
$sqlDB24M = "SELECT
				...
				FROM
					(
						...
						WHERE
							STOCKTRANSACTION.ORDERCODE = '$rowdb21M[PROVISIONALCODE]'
							and STOCKTRANSACTION.ORDERLINE = '$rowdb21M[ORDERLINE]'
							AND STOCKTRANSACTION.LOGICALWAREHOUSECODE = 'M021'
							AND NOT ITXVIEWLAPMASUKGREIGE.ORDERLINE IS NULL
					) AS BLKOKASI";
...
$sqlDB25M = "SELECT
			...
			WHERE
				...
				AND QUALITYDOCUMENTITEMTYPEAFICODE = 'KGF'
				AND QUALITYDOCLINE.QUALITYDOCPRODUCTIONORDERCODE = '$rowdb21M[EXTERNALREFERENCE]'";
...
$sqlDB28M = "SELECT
				...
				WHERE
					p.SUBCODE01 = '$rowdb21M[SUBCODE01]'
					AND p.SUBCODE02 = '$rowdb21M[SUBCODE02]'
					AND p.SUBCODE03 = '$rowdb21M[SUBCODE03]'
					AND p.SUBCODE04 = '$rowdb21M[SUBCODE04]'
...
$sqlDB30M = "SELECT
				...
				WHERE
					s.ORDERCODE = '".$rowdb21M[' PROVISIONALCODE ']."'
					AND s.ORDERLINE = '".$rowdb21M[' ORDERLINE ']."'
				GROUP BY
					s.ORDERCODE,
					s.ORDERLINE,
					e.WIDTHGROSS,
...
$sqlDB21RM = "SELECT
				...
				FROM
					...
					LEFT OUTER JOIN ADSTORAGE a1 ON a1.UNIQUEID = p.ABSUNIQUEID
		...
		while ($rowdb21RM = db2_fetch_assoc($stmt1RM)) {
			...
			$sqlDB23RM = "SELECT
							...
							FROM
								(
									...
									WHERE
										p.ITEMTYPEAFICODE = 'KGF'
										AND p.SUBCODE01 = '" . trim($rowdb21RM[' DECOSUBCODE01 ']) . "'
										AND p.SUBCODE02 = '" . trim($rowdb21RM[' DECOSUBCODE02 ']) . "'
										AND p.SUBCODE03 = '" . trim($rowdb21RM[' DECOSUBCODE03 ']) . "'
										AND p.SUBCODE04 = '" . trim($rowdb21RM[' DECOSUBCODE04 ']) . "'
										AND (
											p.PROJECTCODE = '" . trim($rowdb21RM[' PROJECTCODE ']) . "'
											OR p.ORIGDLVSALORDLINESALORDERCODE = '" . trim($rowdb21RM[' PROJECTCODE ']) . "'
										)
								) a
								LEFT OUTER JOIN PRODUCT p ON p.ITEMTYPECODE = 'GYR'
								AND p.SUBCODE01 = a.SUBCODE01
			...
			$sqlDB28RM = " SELECT
							...
							FROM
								...
								LEFT OUTER JOIN ADSTORAGE a ON a.UNIQUEID = p.ABSUNIQUEID
							WHERE
								p.SUBCODE01 = '" . trim($rowdb21RM[' DECOSUBCODE01 ']) . "'
								AND p.SUBCODE02 = '" . trim($rowdb21RM[' DECOSUBCODE02 ']) . "'
								AND p.SUBCODE03 = '" . trim($rowdb21RM[' DECOSUBCODE03 ']) . "'
								AND p.SUBCODE04 = '" . trim($rowdb21RM[' DECOSUBCODE04 ']) . "'
								AND a.NAMENAME = 'Width'
			...
			$sqlDB29RM = "SELECT
							...
							WHERE
								p.SUBCODE01 = '" . trim($rowdb21RM[' DECOSUBCODE01 ']) . "'
								AND p.SUBCODE02 = '" . trim($rowdb21RM[' DECOSUBCODtE02 ']) . "'
								AND p.SUBCODE03 = '" . trim($rowdb21RM[' DECOSUBCODE03 ']) . "'
								AND p.SUBCODE04 = '" . trim($rowdb21RM[' DECOSUBCODE04 ']) . "'
								AND a.NAMENAME = 'GSM'
								AND p.ITEMTYPECODE = 'KFF'";
				...
				FROM
					(
						...
						FROM
							...
							AND a.NAMENAME = 'StatusRetur'
						WHERE
							s.PROJECTCODE = '".$rowdb21RMBU[' PROJECTCODE ']."'
							AND s.ITEMTYPECODE = 'KGF'
							AND s.DECOSUBCODE02 = '".$rowdb21RMBU[' DECOSUBCODE02 ']."'
							AND s.DECOSUBCODE03 = '".$rowdb21RMBU[' DECOSUBCODE03 ']."'
							AND s.DECOSUBCODE04 = '".$rowdb21RMBU[' DECOSUBCODE04 ']."'
							AND s.LOGICALWAREHOUSECODE = 'M021'
							AND s.LOTCODE = '$rowdb21RMBU[LOTCODE]'
							AND s.TEMPLATECODE = 'OPN'
							AND a.VALUESTRING = '2'
						GROUP BY
							s.ITEMELEMENTCODE
...
$sqlDB23RMBU = "SELECT
				...
				FROM
					(
						...
						WHERE
							p.ITEMTYPEAFICODE = 'KGF'
							AND p.SUBCODE01 = '".trim($rowdb21RMBU[' DECOSUBCODE01 '])."'
							AND p.SUBCODE02 = '".trim($rowdb21RMBU[' DECOSUBCODE02 '])."'
							AND p.SUBCODE03 = '".trim($rowdb21RMBU[' DECOSUBCODE03 '])."'
							AND p.SUBCODE04 = '".trim($rowdb21RMBU[' DECOSUBCODE04 '])."'
							AND (
								p.PROJECTCODE = '".trim($rowdb21RMBU[' PROJECTCODE '])."'
								OR p.ORIGDLVSALORDLINESALORDERCODE = '".trim($rowdb21RMBU[' PROJECTCODE '])."'
							)
					) a
					LEFT OUTER JOIN PRODUCT p ON p.ITEMTYPECODE = 'GYR'
					AND p.SUBCODE01 = a.SUBCODE01
...
$sqlDB28RMBU = "SELECT
				...
				FROM
					...
					LEFT OUTER JOIN ADSTORAGE a ON a.UNIQUEID = p.ABSUNIQUEID
				WHERE
					p.SUBCODE01 = '".trim($rowdb21RMBU[' DECOSUBCODE01 '])."'
					AND p.SUBCODE02 = '".trim($rowdb21RMBU[' DECOSUBCODE02 '])."'
					AND p.SUBCODE03 = '".trim($rowdb21RMBU[' DECOSUBCODE03 '])."'
					AND p.SUBCODE04 = '".trim($rowdb21RMBU[' DECOSUBCODE04 '])."'
					AND a.NAMENAME = 'Width'
...
//Add Stok Flat Knitt				
	...
	$sqlDB21FM = "SELECT
				...
				FROM
					STOCKTRANSACTION s
					LEFT OUTER JOIN ADSTORAGE a ON a.UNIQUEID = s.ABSUNIQUEID
					AND a.NAMENAME = 'StatusFlatKnitt'
					LEFT OUTER JOIN PRODUCTIONDEMAND p ON p.CODE = s.LOTCODE
					LEFT OUTER JOIN ADSTORAGE a1 ON a1.UNIQUEID = p.ABSUNIQUEID
					AND a1.NAMENAME = 'MachineNo'
					LEFT OUTER JOIN FULLITEMKEYDECODER f ON s.FULLITEMIDENTIFIER = f.IDENTIFIER
...
$sqlDB22F1M = "SELECT
				...
				FROM
					...
					LEFT OUTER JOIN DB2ADMIN.ITXVIEWAKJ ITXVIEWAKJ ON SALESORDER.CODE = ITXVIEWAKJ.CODE
				WHERE
					SALESORDER.CODE = '".trim($rowdb21FM[' PROJECTCODE '])."'";
				...
				FROM
					(
						...
						FROM
							...
							AND a.NAMENAME = 'StatusFlatKnitt'
						WHERE
							s.PROJECTCODE = '".$rowdb21FM[' PROJECTCODE ']."'
							AND s.ITEMTYPECODE = 'FKG'
							AND s.DECOSUBCODE02 = '".$rowdb21FM[' DECOSUBCODE02 ']."'
							AND s.DECOSUBCODE03 = '".$rowdb21FM[' DECOSUBCODE03 ']."'
							AND s.DECOSUBCODE04 = '".$rowdb21FM[' DECOSUBCODE04 ']."'
							AND s.LOGICALWAREHOUSECODE = 'M021'
							AND s.LOTCODE = '$rowdb21FM[LOTCODE]'
							AND s.TEMPLATECODE = 'OPN'
							AND a.VALUESTRING = '1'
						GROUP BY
							s.ITEMELEMENTCODE
				...
				FROM
					(
						...
						FROM
							...
							LEFT OUTER JOIN PRODUCTIONRESERVATION p2 ON p.CODE = p2.ORDERCODE
						WHERE
							p.ITEMTYPEAFICODE = 'FKG'
							AND p.SUBCODE01 = '".trim($rowdb21FM[' DECOSUBCODE01 '])."'
							AND p.SUBCODE02 = '".trim($rowdb21FM[' DECOSUBCODE02 '])."'
							AND p.SUBCODE03 = '".trim($rowdb21FM[' DECOSUBCODE03 '])."'
							AND p.SUBCODE04 = '".trim($rowdb21FM[' DECOSUBCODE04 '])."'
							AND (
								p.PROJECTCODE = '".trim($rowdb21FM[' PROJECTCODE '])."'
								OR p.ORIGDLVSALORDLINESALORDERCODE = '".trim($rowdb21FM[' PROJECTCODE '])."'
							)
					) a
					LEFT OUTER JOIN PRODUCT p ON p.ITEMTYPECODE = 'GYR'
					AND p.SUBCODE01 = a.SUBCODE01
...
$sqlDB28FM = "SELECT
				...
				FROM
					...
					LEFT OUTER JOIN ADSTORAGE a ON a.UNIQUEID = p.ABSUNIQUEID
				WHERE
					p.SUBCODE01 = '".trim($rowdb21FM[' DECOSUBCODE01 '])."'
					AND p.SUBCODE02 = '".trim($rowdb21FM[' DECOSUBCODE02 '])."'
					AND p.SUBCODE03 = '".trim($rowdb21FM[' DECOSUBCODE03 '])."'
					AND p.SUBCODE04 = '".trim($rowdb21FM[' DECOSUBCODE04 '])."'
					AND a.NAMENAME = 'Width'
					AND p.ITEMTYPECODE = 'FKF'";
				...
				FROM
					(
						...
						WHERE
							s.PROJECTCODE = '".$proj."'
							AND s.ITEMTYPECODE = 'FKG'
							AND s.DECOSUBCODE02 = '".$rowdb21AN[' DECOSUBCODE02 ']."'
							AND s.DECOSUBCODE03 = '".$rowdb21AN[' DECOSUBCODE03 ']."'
							AND s.DECOSUBCODE04 = '".$rowdb21AN[' DECOSUBCODE04 ']."'
							AND s.LOGICALWAREHOUSECODE = 'M021'
							AND s.LOTCODE = '$rowdb21AN[LOTCODE]'
							AND s.TEMPLATECODE = 'OPN'
							AND a.VALUESTRING = '1'
						GROUP BY
							s.ITEMELEMENTCODE
...
$sqlDB23AN = "SELECT
				...
				FROM
					(
						...
						WHERE
							p.ITEMTYPEAFICODE = 'KGF'
							AND p.SUBCODE01 = '".trim($rowdb21AN[' DECOSUBCODE01 '])."'
							AND p.SUBCODE02 = '".trim($rowdb21AN[' DECOSUBCODE02 '])."'
							AND p.SUBCODE03 = '".trim($rowdb21AN[' DECOSUBCODE03 '])."'
							AND p.SUBCODE04 = '".trim($rowdb21AN[' DECOSUBCODE04 '])."'
							AND (
...
$sqlDB28AN = "SELECT
				...
				FROM
					...
					LEFT OUTER JOIN ADSTORAGE a ON a.UNIQUEID = p.ABSUNIQUEID
				WHERE
					p.SUBCODE01 = '".trim($rowdb21AN[' DECOSUBCODE01 '])."'
					AND p.SUBCODE02 = '".trim($rowdb21AN[' DECOSUBCODE02 '])."'
					AND p.SUBCODE03 = '".trim($rowdb21AN[' DECOSUBCODE03 '])."'
					AND p.SUBCODE04 = '".trim($rowdb21AN[' DECOSUBCODE04 '])."'
					AND a.NAMENAME = 'Width'
			...
			FROM
				...
				LEFT OUTER JOIN (
					...
					GROUP BY
						...
						STOCKTRANSACTION.ITEMELEMENTCODE,
						ITXVIEWBUKMUTGKGKNT.PROJECTCODE
				) prj ON prj.ITEMELEMENTCODE = STOCKTRANSACTION.ITEMELEMENTCODE
			WHERE
				(
...
$sqlDB23K = "SELECT
			...
			FROM
				...
				WHERE
					p.ITEMTYPEAFICODE = 'KGF' 
					AND p.SUBCODE01= '".trim($rowdb21K[' DECOSUBCODE01 '])."' 
					AND p.SUBCODE02 = '".trim($rowdb21K[' DECOSUBCODE02 '])."' 
					AND p.SUBCODE03 = '".trim($rowdb21K[' DECOSUBCODE03 '])."' 
					AND p.SUBCODE04= '".trim($rowdb21K[' DECOSUBCODE04 '])."' $WlotK 
				) a
				LEFT OUTER JOIN PRODUCT p ON p.ITEMTYPECODE = 'GYR' 
				AND p.SUBCODE01= a.SUBCODE01 
				AND p.SUBCODE02= a.SUBCODE02 
...
$sqlDB24K = "SELECT
			...
			WHERE
				(
					pd.PROJECTCODE = '$projectK'
					OR pd.ORIGDLVSALORDLINESALORDERCODE = '$projectK'
				)
				AND p.PRODUCTIONORDERCODE = '$rowdb21K[ORDERCODE]'
			GROUP BY
				pr.LONGDESCRIPTION,
				p.PRODUCTIONORDERCODE,
				pd.SUBCODE01,
